...
class RoomScene {
	constructor(levelName) {
		...
		this.spriteCount = 0;
		this.output = {};
		(this.output.id = uuidv4()), (this.output.sceneType = "SCENE"), (this.output.settings = {
			sceneKey: levelName,
			preloadMethodName: "",
			preloadPackFiles: [],
		...
		if(animations.length > 0) {
			for(animation of animations) {
				...
				fs.writeFileSync(`./out/${className}_animations.json`, JSON.stringify(this.animation_output, null, 4));
			}
		}
		...
		Object.getOwnPropertyNames(this.constructor.prototype).forEach((funcName) => {
			...
			userCode += this.constructor.prototype[funcName].toString() + '\n'
		})
		template = template.replace('userCode', userCode.replace('undefined', ''));
		template = template.replace('className', className);
		fs.writeFileSync(`./out/${className}.js`, template);
		fs.writeFileSync(`./out/${className}.scene`, JSON.stringify(this.output, null, 4));
	}
	...
	text(x, y, texture, frame) {
		this.textCount++;
		let id = uuidv4();
		let image_instance = new GameObject(id, "Text", x, y, texture, frame, `Text_${this.textCount}`);
		return image_instance;
	}
	container(x, y) {
		this.containerCount++;
	...
	getItembyId(item) {
		for(var i = 0; i < this.output.displayList.length; i++) {
			if(this.output.displayList[i].id == item.id) return this.output.displayList[i];
		}
	}
...
class GameObject {
	constructor(id, type, x, y, texture, frame, label) {
		(this.type = type), (this.id = id), (this.components = new Array()), (this.label = label);
		switch(type) {
			case "BitmapText":
			case "Text":
...
class ContainerDecompiler {
	constructor(prefab) {
		...
		this.output = {};
		(this.output.id = uuidv4()), (this.output.sceneType = "PREFAB"), (this.output.settings = {
			sceneKey: className,
			preloadMethodName: "",
			preloadPackFiles: [],
			createMethodName: "_create",
	...
	getItembyId(item) {
		for(let i of this.output.displayList[0].list) {
